...
/** Based on https://pkg.go.dev/www.velocidex.com/golang/oleparse */
...
/**func ParseFile(filename string) ([]*VBAModule, error) {
	...
	if string(signature) == OLE_SIGNATURE {
		...
		macros, err := ParseBuffer(data)
		if err != nil {
			return nil, err
		}

		// Process and print the formatted code for each VBA module
...
func docAnalyze(data []byte) string {

	result := "[+] Office document analysis\n"

	// Check if the data starts with the OLE signature
	if len(data) < len(OLE_SIGNATURE) {
		return fmt.Sprintf("file too small to contain valid OLE signature")
	}

	signature := data[:len(OLE_SIGNATURE)]
	if string(signature) == OLE_SIGNATURE {
		// Parse the buffer as OLE data
		macros, err := ParseBuffer(data)
		if err != nil {
			return fmt.Sprintf("While parsing OLE data: %w", err)
		}
	...
	// Try to interpret the data as a zip file
	reader := bytes.NewReader(data)
	zipReader, err := zip.NewReader(reader, int64(len(data)))
	if err != nil {
		return fmt.Sprintf("While parsing zip file: %w", err)
	}
	...
	for _, f := range zipReader.File {
		if BINFILE_NAME.MatchString(f.Name) {
			rc, err := f.Open()
			if err != nil {
				return fmt.Sprintf("While opening zip file: %w", err)
			}
			defer rc.Close()

			fileData, err := io.ReadAll(rc)
			if err != nil {
				return fmt.Sprintf("While reading zip file: %w", err)
			}
			modules, err := ParseBuffer(fileData)
			if err == nil {
				results = append(results, modules...)
			}
		}
...
func AnalyzeWrapper(this js.Value, args []js.Value, fileBytes []byte) any {

	//fileBytes := make([]byte, args[0].Get("length").Int())
	js.CopyBytesToGo(fileBytes, args[0])

	result := docAnalyze(fileBytes)
